...
class CronExpressionParser {
    ...
    static parse(expression, options = {}) {
        ...
        const currentDate = options.currentDate || new CronDate_1.CronDate(undefined, 'UTC');
        ...
        const second = CronExpressionParser.#parseField(CronUnit.Second, rawFields.second, fields_1.CronSecond.constraints);
        const minute = CronExpressionParser.#parseField(CronUnit.Minute, rawFields.minute, fields_1.CronMinute.constraints);
        const hour = CronExpressionParser.#parseField(CronUnit.Hour, rawFields.hour, fields_1.CronHour.constraints);
        const month = CronExpressionParser.#parseField(CronUnit.Month, rawFields.month, fields_1.CronMonth.constraints);
        const dayOfMonth = CronExpressionParser.#parseField(CronUnit.DayOfMonth, rawFields.dayOfMonth, fields_1.CronDayOfMonth.constraints);
        const { dayOfWeek: _dayOfWeek, nthDayOfWeek } = CronExpressionParser.#parseNthDay(rawFields.dayOfWeek);
        const dayOfWeek = CronExpressionParser.#parseField(CronUnit.DayOfWeek, _dayOfWeek, fields_1.CronDayOfWeek.constraints);
        const fields = new CronFieldCollection_1.CronFieldCollection({
            second: new fields_1.CronSecond(second, ['*', '?'].includes(rawFields.second)),
            minute: new fields_1.CronMinute(minute, ['*', '?'].includes(rawFields.minute)),
            hour: new fields_1.CronHour(hour, ['*', '?'].includes(rawFields.hour)),
            dayOfMonth: new fields_1.CronDayOfMonth(dayOfMonth, ['*', '?'].includes(rawFields.dayOfMonth)),
            month: new fields_1.CronMonth(month, ['*', '?'].includes(rawFields.month)),
            dayOfWeek: new fields_1.CronDayOfWeek(dayOfWeek, ['*', '?'].includes(rawFields.dayOfWeek)),
        });
        return new CronExpression_1.CronExpression(fields, { ...options, expression, currentDate, nthDayOfWeek });
    }
    /**
